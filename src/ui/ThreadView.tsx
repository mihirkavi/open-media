  ...
  if (!conversation) {
    return (
      <View style={styles.placeholder}>
        <View style={styles.placeholderIcon}><Ionicons name="chatbubbles-outline" size={27} color={colors.accent} /></View>
        <Text style={styles.placeholderTitle}>Your conversations, together</Text>
        <Text style={styles.placeholderCopy}>Select a conversation to open the thread.</Text>
      </View>
    );
  }
    ...
    <KeyboardAvoidingView style={styles.panel} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.header}>
        <View style={styles.headerLeading}>
          {isCompact ? (
            <Pressable accessibilityLabel="Back to conversations" onPress={onBack} style={styles.iconButton}>
              <Ionicons name="chevron-back" size={22} color={colors.accent} />
            </Pressable>
          ) : null}
          <View style={styles.headerAvatarStack}>
            {conversation.participants.slice(0, 2).map((participant, index) => (
              <View key={participant.id} style={[styles.headerAvatar, { backgroundColor: participant.avatarColor, marginLeft: index ? -10 : 0, zIndex: 2 - index }]}>
                <Text style={styles.headerAvatarText}>{participant.initials}</Text>
              </View>
            ))}
          </View>
          <View style={styles.headerCopy}>
            <Text numberOfLines={1} style={styles.headerTitle}>{recipientNames}</Text>
            <View style={styles.headerMeta}>
              <View style={[styles.sourceDot, { backgroundColor: conversation.source === 'gmail' ? colors.gmail : colors.icloud }]} />
              <Text style={styles.headerSubtitle}>{conversation.source === 'gmail' ? 'Gmail' : 'iCloud Mail'} · {conversation.subject}</Text>
            </View>
          </View>
        </View>
        <View style={styles.headerActions}>
          ...
          <Pressable accessibilityLabel="Conversation details" style={styles.iconButton}><Ionicons name="information-circle-outline" size={21} color={colors.textSecondary} /></Pressable>
        </View>
      </View>
      ...
      <ScrollView style={styles.messages} contentContainerStyle={styles.messagesContent}>
        <View style={styles.subjectCard}>
          ...
          <Text style={styles.subjectTitle}>{conversation.subject}</Text>
          <Text style={styles.subjectParticipants}>To: {recipientNames}</Text>
        </View>
        ...
        {conversation.messages.map((message, index) => (
          <MessageBubble
            key={message.id}
            message={message}
            showSender={index === 0 || conversation.messages[index - 1].sender.id !== message.sender.id}
          />
        ))}
      </ScrollView>
      ...
      <View style={styles.composerWrap}>
        <View style={styles.demoNotice}>
          ...
          <Text style={styles.demoNoticeText}>Demo mode · nothing is connected or sent</Text>
        </View>
        <View style={styles.composer}>
          <Pressable accessibilityLabel="Add attachment" style={styles.composerIcon}><Ionicons name="add" size={23} color={colors.textSecondary} /></Pressable>
          <TextInput
            accessibilityLabel="Message"
            multiline
            placeholder={`Message ${conversation.participants[0].name}`}
          ...
          />
          <Pressable
            accessibilityLabel="Send message (demo only)"
            disabled={!draft.trim()}
            onPress={() => setDraft('')}
...
function MessageBubble({ message, showSender }: { message: Message; showSender: boolean }) {
  ...
  return (
    <View style={[styles.messageGroup, outbound ? styles.messageGroupOutbound : styles.messageGroupInbound]}>
      {showSender && !outbound ? <Text style={styles.senderName}>{message.sender.name}</Text> : null}
      ...
      <Text style={[styles.messageTime, outbound && styles.messageTimeOutbound]}>
        {time}{outbound ? ' · Delivered' : ''}
      </Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  headerSubtitle: { flex: 1, color: colors.textTertiary, fontSize: 10 },
  ...
  messages: { flex: 1 },
  messagesContent: { width: '100%', maxWidth: 760, alignSelf: 'center', paddingHorizontal: 24, paddingTop: 24, paddingBottom: 16 },
  subjectCard: { alignSelf: 'stretch', padding: 16, marginBottom: 22, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: radii.medium },
  subjectLabel: { color: colors.textTertiary, fontSize: 9, fontWeight: '800', letterSpacing: 1.1, marginBottom: 5 },
  subjectTitle: { color: colors.text, fontSize: 16, fontWeight: '700' },
  subjectParticipants: { color: colors.textSecondary, fontSize: 11, marginTop: 4 },
  dateDivider: { alignSelf: 'center', marginBottom: 18, color: colors.textTertiary, fontSize: 10, fontWeight: '600' },
  messageGroup: { maxWidth: '78%', marginBottom: 14 },
  ...
  messageTime: { marginTop: 4, marginHorizontal: 8, color: colors.textTertiary, fontSize: 9 },
  messageTimeOutbound: { textAlign: 'right' },
  ...
  composerIcon: { width: 34, height: 34, borderRadius: 17, alignItems: 'center', justifyContent: 'center' },
